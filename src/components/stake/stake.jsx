...
class Stake extends Component {
    ...
    return (
      <div className={classes.root}>
        ...
        <div className={classes.overview}>
          ...
          <div className={classes.overviewField}>
            <Typography variant={'h3'} className={classes.overviewTitle}>Remaining pYFARMER</Typography>
  ...
  renderStakePools = (assets) => {
    ...
    return (
      <div className={classes.actions}>
        ...
        <Typography className={classes.stakeTitle} variant={'h3'}>pYFARMER to receive: {assets.totalStaked} </Typography>
  ...
  calcpYfarmAmount = (amount) => {
    const { pool } = this.state
    const tokens = pool.tokens
    tokens[0].totalStaked = amount / 10;
  }
  ...
  onChange = (value, event) => {
    ...
    if(parseFloat(event.target.value) < parseFloat(value)){
      ...
      val[event.target.id] = value > parseFloat(event.target.value) ? event.target.value : (value + '')
      this.setState(val)
      this.calcpYfarmAmount(event.target.value);
    } else {
      console.log("value bigger");
      let val = []
      val[event.target.id] = event.target.value > parseFloat(event.target.value) ? event.target.value : (event.target.value + '')
      this.setState(val)
      this.calcpYfarmAmount(value);
    }
  }
